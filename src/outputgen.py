...
class OutputGen:
    ...
    def GenImage(self, surface, nowBlock):
        ...
        for t in range(self.blockMin, self.blockMax, 12):
            if (t == nowBlock):
                pygame.draw.line(surface, black, (self.BlockX(t), 235), (self.BlockX(t), 60))
            else:
                self.SmallDotLine(surface, (self.BlockX(t), 235), (self.BlockX(t), 60), black)

            
            timetext = dt.datetime.fromtimestamp(GetBlockStartTime(t)).strftime("%H:%M")
            txtsurf = smallfont.render(timetext, False, black)
            surface.blit(txtsurf,(self.BlockX(t) - txtsurf.get_width() // 2, 225 + txtsurf.get_height()))
        ...
        futureTemps = self.predictor.PredictFutureTemps(GetBlockStartTime(nowBlock), self.heating, self.blockMax + 1 )
        ...
        for b in range(self.blockMin, nowBlock):
            x, y = self.ChartCoord(b, self.sensors.GetTempAtTime(GetBlockStartTime(b)), minTemp, maxTemp)
            x2, y2 = self.ChartCoord(b + 1, self.sensors.GetTempAtTime(GetBlockStartTime(b + 1)), minTemp, maxTemp)
        ...
        timetxt = dt.datetime.fromtimestamp(GetBlockStartTime(nowBlock)).strftime("%H:%M")
